<template>
  <div class="sales-board">
      ...
      <check-order :isShowCheckDialog="isShowCheckOrder" :order-id="orderId"
      @on-close-check-dialog="hideCheckOrder"></check-order>

      <my-dialog :is-show="isShowErrDialog" @onclose="hideErrDialog">
  ...
  export default {
    ...
    methods:{
      ...
      getPrice(){
        ...
        this.$http.post('http://127.0.0.1:9527/getPrice', reqParams)
        .then((res) => {
          let b = res.data.body.buyNum
          let bt = res.data.body.buyType
          let p = res.data.body.period
          let v = 10
          this.price = (b+(b*bt)+(b*p)+(b*v))*10
        })
      },
      showPayDialog(){
      ...
      confirmBuy(){
        ...
        this.$http.post('http://127.0.0.1:9527/createOrder', reqParams)
        .then((res)=>{
          this.isShowPayDialog = false/*关闭确认购买页面*/
